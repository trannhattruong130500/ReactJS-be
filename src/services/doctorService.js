    ...
    return new Promise(async (resolve, reject) => {
        try {
            if (!data.arrSchedule || !data.doctorId || !data.formatedDate) {
                ...
                resolve({
                    errCode: 0,
                    errMessage: 'Upload data Succeed'
                })
            }
        } catch (e) {
            reject(e);
    ...
    return new Promise(async (resolve, reject) => {
        try {
            if (!doctorId || !date) {
                ...
                let validBooking = await db.Booking.findOne({
                    where: {
                        doctorId: doctorId,
                        date: date
                    },
                    ...
                    include: [
                        {
                            ...
                            attributes: [
                                'email', 'firstName', 'address', 'gender', 'phoneNumber'
                            ],
                            include: [
                                {
...
let sendRemedy = (data) => {
    return new Promise(async (resolve, reject) => {
        try {
            if (!data.doctorId || !data.patientId || !data.email || !data.timeType || !data.imgBase64) {
                resolve({
                    errCode: 1,
                    errMessage: 'Missing required paremeters'
                })
